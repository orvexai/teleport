...
// OIDC SSO proxy handlers for the community (OSS) build.
// These mirror the GitHub trio (githubLoginWeb, githubCallback,
// githubLoginConsole) in lib/web/apiserver.go and implement decision D7.
...
// oidcLoginWeb redirects the browser to the OIDC IdP authorization endpoint.
...
// oidcLoginConsole handles the console (CLI) OIDC login request, returning the
// IdP redirect URL to the tsh client. Mirrors githubLoginConsole.
...
// oidcCallback handles the OIDC IdP callback after the user authenticates.
...
func (h *Handler) oidcCallback(w http.ResponseWriter, r *http.Request, p httprouter.Params) string {
	...
	if err != nil {
		...
		// Try to find the auth request to get the original client redirect URL and
		// terminate the failed flow immediately.
	...
	// If we created a web session, set session cookie and redirect to original URL.
